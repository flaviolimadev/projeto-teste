    ...
    @section('conteudo')
        ...
        <style>
            .header-item-op i.fa-sharp.fa-solid.fa-plus {
                font-size: 16px;
            }
            body .header-item-op i.fa-sharp.fa-solid.fa-plus {
                color: #888ea8 !important;
            }
            body.dark .header-item-op i.fa-sharp.fa-solid.fa-plus {
                color: #e0e6ed !important;
            }
        </style>
        ...
        @foreach($tips as $tip)
            <!-- Modal Tutorial -->
            <div class="modal fade" id="modalTutorial{{ $tip->id }}" tabindex="-1" aria-labelledby="modalTutorial{{ $tip->id }}" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content" style="border-radius: 20px; max-height: 800px; overflow-y: auto;">
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close" style="position: absolute; top: 10px;right: 10px; opacity: 1; z-index: 1;"></button>

                        <img src="{{ asset('storage') }}/{{ $tip->avatar }}" alt="Informações" width="100%">

                        <p class="tag-marcadagua-modal" style="background-color: red;">ANTIRED © usuário: {{ $AuthUser->email }} </p>
                    </div>
                </div>
            </div>
        @endforeach
        ...
        <div class="row layout-top-spacing">
            <div class="area-operacoes">
                <div class="itens-operacoes">
                    ...
                    <div class="item-op">
                        <div class="header-item-op">
                            <div>
                                <a href="{{ route('newSinal', [
                                    ...
                                    'odd_02' => $tip->odds02,
                                    'evento' => $tip->liga,
                                    'mercado_01' => $tip->descricao01,
                                    'mercado_02' => $tip->descricao02,
                                    'data' =>  $tip->quando,
                                    'lucro' => $tip->lucro,
                                ]) }}">
                                    <i class="fa-sharp fa-solid fa-plus" data-bs-toggle="tooltip" data-bs-title="Adicionar"></i>
                                </a>
                        ...
                        <div class="body-item-op">
                            <div class="left">
                                <p><a href="{{ $tip->link01 }}" target="_blank">{{ $tip->casa01 }}</a></p>
                                <p><a href="{{ $tip->link02 }}" target="_blank">{{ $tip->casa02 }}</a></p>
                                <p>{{ explode('X', $tip->confronto)[0] }}</p>
                                <p>{{ explode('X', $tip->confronto)[1] }}</p>
                                <p>{{$tip->descricao01}}</p>
                                <p>{{$tip->descricao02}}</p>
                            </div>
                            <div class="right">
                                ...
                                <div>
                                    ...
                                    <span class="odd">
                                        <input class="odd" type="text" id="valora{{ $tip->id }}" value="{{ $tip->odds01 }}">
                                    </span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <span class="odd" id="valor02">
                                        <input class="odd" type="text" id="valorb{{ $tip->id }}" value="{{ $tip->odds02 }}">
                                    </span>
                                </div>
                        ...
                        <div class="footer-item-op">
                            ...
                            <button class="btn-calc-op" type="button" data-bs-toggle="modal" data-bs-target="#modalCalculadora" onclick="calc('valora{{ $tip->id }}', 'valorb{{ $tip->id }}', '{{ explode('X', $tip->confronto)[0] }}', '{{ explode('X', $tip->confronto)[1] }}', '{{ $tip->quando }}')">
                                <span>CALCULAR</span>
                    ...
                    <div class="box-tutorial-mobile">
                        <button type="button" data-bs-toggle="modal" data-bs-target="#modalTutorial{{$tip->id}}">Ver tutorial</button>
                    </div>
                ...
                <div class="itens-tutorial">
                    <p>Tutorial</p>

                    @foreach($tips as $tip)

                    <div class="box-tutorial" style="background-image: url({{ asset('storage') }}/{{ $tip->avatar }})">
                        <button type="button" data-bs-toggle="modal" data-bs-target="#modalTutorial{{$tip->id}}">Ver tutorial</button>
                    </div>
    ...
    const meuInput = document.getElementById("valor-investir");

    meuInput.addEventListener("input", function(event) {
    // Ação a ser executada quando o valor do input for modificado
    });
</script>
